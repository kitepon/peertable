...
- alarm／seat-status recordが進捗時刻を更新しないため、周期ensureが生きたbridgeを死んだものとして再起動する。Windowsでは旧processがlogを保持したままlog切詰めを先行し、`EBUSY`も発生する。
...
- alarm／seat-statusも各tickで共通の`last_progress_at`を原子的に更新し、wakeupと同じ生存判定を使う。再起動が必要な場合はsession停止をlog切詰めより先に行う。
...
- 生きたbridgeへensureを再実行してもpidが変わらず、Windowsで`EBUSY`を出さない。
