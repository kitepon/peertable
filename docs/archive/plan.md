...
- **決定70（2026-08-10・オーナー裁定）: Peertable は dotagents 工場の管理対象へ編入され、push が既定になる**
  ...
  Peertable は独立製品のまま dotagents 開発工場の統合対象へ編入された。
  wire v7 の固定14製品に `peertable` が含まれ、dotagents 側の統合契約は
  [ADR 0127](https://github.com/kitepon/dotagents/blob/main/docs/adr/0127-wire-v7-peertable-enrollment.md)
...
詳細は `docs/archive/plan_roundtable-ux2-20260810.md`「背景」節）。以下は 2026-08-10 に file:line で裏取りして
...
メンバー着席の製品内正本は、source commitとschemaを記録した同梱 [skill/02_models.snapshot.md](../../skill/02_models.snapshot.md) の役割→順位表である。dotagentsの表はsnapshot更新時のprovenanceであり、隣接repoが存在するだけでは読まない。外部表を使うのは`PEERTABLE_MODELS_DOC`または`DOTAGENTS_ROOT`を明示した時だけとする。
`launch-seat.sh` は役割名を必須にし、未指定・空・未知（旧 `worker`/`auditor` を含む）を着席前に拒否する。
model / harness / effort は順位表の着席可能な最上位から機械解決する。呼び出し側が三者を渡して正本を迂回する経路は、席の再起動上書きに限る。
...
**決定126: roomログ出力はMCP結果上限より小さくページングする。** `read_log`既定50件がLiveTRで21.8KBとなり、GrokのMCP出力上限20KBを超えてtool callが停止した。`read_log`は直近20件かつUTF-8 12KB、`read_unread`も12KB単位にし、未返却分を既読へ進めず再読案内を返す。修正後の実Grok席でRoom Members／Read Unreadが秒内に完了し監査が継続した。patch `0.8.25`。

**決定127: 親post入口も共通token解決を使う。** `post-message.mjs`だけが`PEERTABLE_POST_TOKEN`の明示exportを要求し、設定fileに正規tokenがあっても403になった。bridge群と同じ`resolvePostToken`へ一本化し、親が事前export順を組み立てず1回のpost呼出しで送れる形にする。patch `0.8.26`。

**決定128: 長いCodex approvalは現在footerを根拠に全画面から判定する。** 質問本文が24行を超えて末尾窓から外れ、現在の`Press enter to confirm or esc to cancel`だけが残る承認画面を常駐処理が検出できなかった。現在footerが末尾に在る時だけ全画面から質問を読み、busy／通常composerが現在なら古いscrollbackを優先しない。patch `0.8.27`。

## 67. 状態名と現在作業を点の横へ常設する（2026-08-29・決定129）

**決定129: 席の存在・点灯・実作業を同じ意味にしない。** Web UIの参加者一覧は色点だけでなく、server生成の実効状態を「作業中／待機／承認待ち／停止／不明」と文字で常設する。現在作業もホバー内へ隠さず常設する。

現在作業はserverが`activity_text`／`activity_at`として一元生成する。`busy`だけ、本人の最新の`[次の行動]`／`[claim]`／`[引受]`／`[作業中]`を採る。申告が無ければ「作業内容未報告」とする。`idle`は「待機中」、`blocked`は「承認操作待ち」、`dead`は「セッション停止」、`unknown`は「稼働状態を確認できません」とし、古いmissionや完了済み発言を現在形で再表示しない。発言SSEで活動申告を受けた時はmember一覧を即時更新する。API能力は`member_activity_v1`。patch `0.8.30`。
