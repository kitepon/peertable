...
## 63. Windows native bridge起動とdoctorのshim解決をPowerShell経路へ統一する（2026-08-29・決定113）

**決定113: psmuxが所有するWindows nativeの常駐shellへ、POSIX command文字列を渡さない。** `ensure-bridge.sh`はPowerShell 7の`-EncodedCommand`を生成し、Windows node実体、Windows path、必要env、bridge引数、log redirectionをPowerShell構文で渡す。POSIX環境の既存`env ... node ...`経路は変更しない。
...
focused reproはPowerShell子processで空白・日本語・引用符を含むenv／引数／pathとlog生成を実行し、shim解決はtodo／project statusの両argvを固定する。patch `0.8.16`。
