...
**mission はフェーズ・campaign 全体での自分の担いを書く欄である**（例:「atlas-casework の受入監査担当」
「fact-watch 実装の主担当」）。**今やっている作業・待機状態・進捗を書かない**——現在作業は room 発言
（[claim]/[引受] 等）からサーバーが機械導出して別に表示する（オーナー裁定 2026-08-30:
「ミッションはフェーズや工程全体として見たときのミッション。今やってることを書く場所じゃない」）。
着席時の mission は起動スナップショットである。campaign / plan が次のフェーズへ進んだら、
自分で更新する。親に依頼しない。席は再起動しない。更新は**フェーズの変わり目だけ**であり、
工程・作業のたびに書き換えない。
